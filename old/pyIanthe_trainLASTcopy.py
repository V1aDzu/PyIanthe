...
import random
import numpy as np
# from torch.cuda.amp import GradScaler # Раскомментировать, если используете!
    ...
    def save_checkpoint(
        ...
        optimizer, 
        scheduler, 
        scaler=None, # Необязательный аргумент для GradScaler
        epoch=None, 
        global_step=None
    ):
        """
        Сохраняет полный чекпоинт обучения в один файл.
        
        Args:
            filename (str): Путь к файлу для сохранения.
            model (nn.Module): Обучаемая модель.
            optimizer (torch.optim.Optimizer): Оптимизатор.
            scheduler (torch.optim.lr_scheduler._LRScheduler): Планировщик скорости обучения.
            scaler (Optional[GradScaler]): Объекта GradScaler (для смешанной точности).
            epoch (Optional[int]): Текущая эпоха.
            global_step (Optional[int]): Текущий глобальный шаг/итерация.
        """
        
        # 1. Сбор всех состояний RNG
        rng_states = {
        ...
        # 2. Сбор всех состояний обучения
        checkpoint = {
            'epoch': epoch,
        ...
        # 3. Добавление состояния GradScaler (если он используется)
        if scaler is not None:
            # Убедитесь, что вы передаете GradScaler, а не bool
            if isinstance(scaler, torch.cuda.amp.GradScaler):
                checkpoint['scaler_state_dict'] = scaler.state_dict()
            else:
                print("⚠️ Предупреждение: Параметр 'scaler' не является GradScaler. Пропускаю его сохранение.")
                
        # 4. Сохранение файла
        torch.save(checkpoint, filename)
        print(f"✅ Полный чекпоинт обучения сохранен в {filename}. Шаг: {global_step}, Эпоха: {epoch}")
    ...
    def load_checkpoint(
        ...
        """
        Загружает полный чекпоинт обучения из файла и восстанавливает все состояния.
        
        Args:
            filename (str): Путь к файлу для загрузки.
            model (nn.Module): Модель (будет загружено состояние).
            optimizer (torch.optim.Optimizer): Оптимизатор (будет загружено состояние).
            scheduler (torch.optim.lr_scheduler._LRScheduler): Планировщик (будет загружено состояние).
            scaler (Optional[GradScaler]): Объекта GradScaler (будет загружено состояние).
            
        Returns:
            tuple: (epoch, global_step) или (None, None), если файл не найден.
        """
        
        if not os.path.exists(filename):
            print(f"❌ Ошибка: Файл чекпоинта {filename} не найден.")
            return None, None
            
        # Загружаем чекпоинт
        checkpoint = torch.load(filename, map_location='cuda' if torch.cuda.is_available() else 'cpu')
        
        # 1. Восстановление состояний модели, оптимизатора, шедулера
        model.load_state_dict(checkpoint['model_state_dict'])
        optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        scheduler.load_state_dict(checkpoint['scheduler_state_dict'])
        
        # 2. Восстановление GradScaler (если он был сохранен и передан)
        if scaler is not None and 'scaler_state_dict' in checkpoint:
            # Убедитесь, что вы передаете GradScaler, а не bool
            if isinstance(scaler, torch.cuda.amp.GradScaler):
                scaler.load_state_dict(checkpoint['scaler_state_dict'])
            else:
                print("⚠️ Предупреждение: В чекпоинте есть scaler, но переданный объект не GradScaler. Пропускаю его восстановление.")

        # 3. Восстановление состояний RNG (самое важное для воспроизводимости)
        ...
        print(f"✅ Полный чекпоинт успешно восстановлен из {filename}. Продолжение с Шага: {global_step}, Эпохи: {epoch}")
 ...
 #       try:
  #          rng_states0 = torch.load("cuda0_rng.pt")
   #         rng_states1 = torch.load("cuda1_rng.pt")
    ...
    # Ініціалізація Scaler для змішаної точності (якщо потрібно)
    scaler = None
    if fp16 and not bf16: # GradScaler потрібен тільки для FP16, але не для BF16/Native AMP
        from torch.cuda.amp import GradScaler
        scaler = GradScaler()
        logger.info("✓ GradScaler ініціалізовано для FP16.")
    elif bf16:
         logger.info("✓ Використовується BF16 (Native AMP), GradScaler не потрібен.")
...
# Після цього ви можете викликати trainer.train()
    ...
    # 10. Ініціалізація оптимізатора та шедулера для зовнішнього чекпоінту
    if trainer.optimizer is None or trainer.lr_scheduler is None:
        trainer.create_optimizer_and_scheduler(num_training_steps=trainer.num_training_steps)
        logger.info("✓ Оптимізатор та Планувальник створені Trainer.")
    ...
    for epoch in range(EPOCHS):
        ...
        #  Отримуємо поточний стан тренера
            ...
            save_checkpoint(
                os.path.join(emergency_checkpoint, "checkpoint.pth"),
                model,
                optimizer,
                scheduler,
                scaler=scaler,
